...
def load_users_from_file():
    try:
        ...
        logging.error("Users file not found, starting with an empty list.")
        return []
...
async def cmd_start(message: types.Message, state: FSMContext):
    # Сбрасываем состояние FSM до начального, если оно было установлено
    if state is not None:
        await state.finish()

    # Проверяем, зарегистрирован ли уже пользователь, и добавляем его ID в файл, если нет
    user_id = str(message.from_user.id)  # Преобразование ID пользователя в строку для JSON
    if user_id not in users:  # Проверка на наличие пользователя в списке
        users.append(user_id)  # Добавление пользователя в список
        save_users_to_file(users)  # Сохранение обновленного списка пользователей в файл
        await message.answer("Вы успешно зарегистрированы.")

    # Создание клавиатуры для выбора действий пользователем
    keyboard = types.ReplyKeyboardMarkup(resize_keyboard=True)
    ...
    await EventStates.Start.set()  # Устанавливаем начальное состояние в FSM
        ...
        for event_name, participants in events.items():
            ...
            if participants:
                ...
                for idx, participant in enumerate(participants, start=1):
                    response_message += f"{idx}. 👤 {participant['name']}\n"
            else:
